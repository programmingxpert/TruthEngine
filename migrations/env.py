"""Alembic environment configuration for TruthEngine migrations."""

from logging.config import fileConfig

from alembic import context

from truthengine.core.config import get_settings
from truthengine.core.database import Base
from truthengine.investigations.candidates.persistence import (
    CandidatePassageRecord,
    DocumentSegmentRecord,
)
from truthengine.investigations.graphs.persistence import (
    ClaimRecord,
    EvidenceGraphRecord,
    EvidenceItemRecord,
    EvidenceRelationRecord,
)
from truthengine.investigations.persistence import (
    InvestigationRecord,
    TimelineEventRecord,
)
from truthengine.investigations.planning.persistence import (
    InvestigationPlanRecord,
)
from truthengine.sources.persistence import (
    SourceRecord,
    SourceSnapshotRecord,
)

config = context.config

if config.config_file_name is not None:
    fileConfig(config.config_file_name)

target_metadata = Base.metadata


def get_database_url() -> str:
    """Return the configured database URL for Alembic."""
    return get_settings().database_url


def run_migrations_offline() -> None:
    """Run migrations without creating a database connection."""
    context.configure(
        url=get_database_url(),
        target_metadata=target_metadata,
        literal_binds=True,
        dialect_opts={"paramstyle": "named"},
    )

    with context.begin_transaction():
        context.run_migrations()


def run_migrations_online() -> None:
    """Run migrations with an active database connection."""
    from sqlalchemy import engine_from_config, pool

    configuration = config.get_section(config.config_ini_section, {})
    configuration["sqlalchemy.url"] = get_database_url()
    connectable = engine_from_config(
        configuration,
        prefix="sqlalchemy.",
        poolclass=pool.NullPool,
    )

    with connectable.connect() as connection:
        context.configure(connection=connection, target_metadata=target_metadata)
        with context.begin_transaction():
            context.run_migrations()

    connectable.dispose()


# Referencing the models keeps static analyzers from removing the imports above
# while still making the metadata registration explicit for Alembic.
_ = (
    InvestigationRecord,
    TimelineEventRecord,
    InvestigationPlanRecord,
    ClaimRecord,
    EvidenceGraphRecord,
    EvidenceItemRecord,
    EvidenceRelationRecord,
    SourceRecord,
    SourceSnapshotRecord,
    DocumentSegmentRecord,
    CandidatePassageRecord,
)

if context.is_offline_mode():
    run_migrations_offline()
else:
    run_migrations_online()
